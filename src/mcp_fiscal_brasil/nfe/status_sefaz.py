...
- Estado com estrutura propria (AM, BA, GO, MG, MS, MT, PE, PR, SP).
...
(nfephp-org/sped-nfe, storage/wsnfe_4.00_mod55.xml), cross-referenciada com o
agrupamento SVRS/SVAN publicado pelas proprias SEFAZ. Todas as chamadas
exigem mTLS com certificado A1 (nao ha consulta de status sem certificado -
e uma exigencia da camada de transporte dos webservices SEFAZ, nao apenas
de operacoes que gravam dados).
"""
...
from .._core.errors import FiscalConfigurationError, FiscalHTTPError
...
# Cache do SSLContext montado a partir do certificado A1: evitar recarregar o
# .pfx do disco a cada UF consultada (uma varredura completa consulta ~11
# endpoints distintos). Valido para o tempo de vida do processo; um novo
# certificado exige reiniciar o servico.
...
def _obter_ssl_context(caminho_certificado: str, senha: str) -> ssl.SSLContext:
    ...
    if _ssl_context_cache is None:
        ...
        _ssl_context_cache = criar_ssl_context_em_memoria(chave_pem, cert_pem, chain_pem)
...
async def consultar_status_real(
    ...
    Raises:
        FiscalConfigurationError: certificado nao configurado ou UF sem endpoint mapeado.
        FiscalValidationError: certificado invalido (senha errada, arquivo corrompido).
    ...
    ssl_ctx = _obter_ssl_context(caminho_certificado, senha)
    body_content = _montar_body(uf, ambiente)

    logger.info("sefaz_status_real_started", uf=uf, ambiente=ambiente, endpoint=endpoint)

    body_el = await _enviar_soap_mtls(endpoint, body_content, ssl_ctx, namespace=_NS_NFE, timeout=timeout)
...
def obter_status_certificado(
    ...
    Usado pelo endpoint GET /v1/fiscal/certificado/status: o painel Lumiere so
    ve "configurado / titular / validade", nunca o arquivo ou a senha.
    """
    if not caminho_certificado or not senha:
